...
namespace jj08
{
    void test_unordered_multiset(long& value)
    {
        ...
        cout << "milli-seconds:" << (clock() - timeStart) << endl; //unordered_multiset插入1000000个随机数据所使用的时间:4406ms
        cout << "unordered_multiset.size() = " << c.size() << endl;//1000000
        cout << "unordered_multiset.max_size() = " << c.max_size() << endl;//357913941
        cout << "unordered_multiset.bucket_count() = " << c.bucket_count() << endl;//篮子的个数（其实就是每个链表前面的格子，一定比元素多）：1056323
        cout << "unordered_multiset.load_factor() = " << c.load_factor() << endl;//载重因子：0.94668
        cout << "unordered_multiset.max_load_factor() = " << c.max_load_factor() << endl;//最大的载重因子恒定为1
        cout << "unordered_multiset.max_bucket_count() = " << c.max_bucket_count() << endl;//最大篮子数量：357913941
